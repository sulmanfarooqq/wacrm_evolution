...
// Validation assertions run BEFORE the network call. We stub fetch to a
// never-resolving mock so a test that accidentally falls through to the
// request body would hang (and fail) rather than silently hit the
// Evolution API.
const neverFetch = () =>
  ...
  it("sends the right payload shape when all inputs are valid", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (url: string, init: RequestInit) => {
        ...
        return new Response(JSON.stringify({ key: { id: "wamid.PASS" } }), {
          status: 200,
        });
      }),
    );
    ...
    expect(captured!.body).toEqual({
      ...
      footer: "Tap one",
      buttons: [
        { title: "reply", displayText: "Yes", id: "yes" },
        { title: "reply", displayText: "No", id: "no" },
      ],
    });
  });
});
  ...
  it("sends the right payload shape when valid", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (url: string, init: RequestInit) => {
        ...
        return new Response(JSON.stringify({ key: { id: "wamid.LIST" } }), {
          status: 200,
        });
      }),
    );
      ...
      values: [
        {
          title: "Orders",
          rows: [
            { title: "Order #1", description: "€12", rowId: "order_1" },
            { title: "Order #2", description: "", rowId: "order_2" },
          ],
        },
      ],
    });
