...
export interface WhatsAppConfig {
  id: string;
  user_id: string;
  /** Evolution instance name (the column is reused from Meta's
   *  phone-number id so existing senders keep compiling). */
  phone_number_id: string;
  instance_name?: string;
  /** Base URL of the self-hosted Evolution API (no trailing slash). */
  api_base_url?: string;
  /** 'evolution' for the self-hosted API (only supported value). */
  provider?: string;
  waba_id?: string;
  access_token: string;
  verify_token?: string;
  ...
  connected_at?: string;
  /** Set when POST /webhook/set last succeeded on the Evolution
   *  instance. NULL means the instance is reachable but Evolution
   *  won't forward inbound events to this app. */
  webhook_configured_at?: string;
  /** Legacy Meta registration fields — unused for Evolution, kept for
   *  schema parity. */
  registered_at?: string;
  subscribed_apps_at?: string;
  /** Last webhook-configure error; cleared on success. */
